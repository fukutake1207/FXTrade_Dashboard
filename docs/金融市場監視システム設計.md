...
| 更新頻度 | セッション毎（1日3回） |
| ニュースソース | 汎用（特定指定なし） |
| 生成エンジン | Gemini API（デフォルト）/ Claude API（切替対応） |
| 切替方式 | 環境変数 `NARRATIVE_PROVIDER=gemini|claude` |
...
#### 2.4.4 AIプロンプト設計（Gemini/Claude 共通）
...
| **AI** | Gemini API（gemini-2.5-flash）/ Claude API | ナラティブ生成（切替対応） |
...
| AI | Gemini API (Google) / Claude API (Anthropic) | gemini-2.5-flash / claude-sonnet-4-5 |
...
apis:
  narrative_provider: "gemini"  # gemini | claude
  claude:
